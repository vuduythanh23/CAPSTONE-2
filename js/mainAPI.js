...
function renderShoeCarousel(shoes) {
  ...
  for (let i = 0; i < shoes.length; i += chunkSize) {
    ...
    backgroundDiv.style.backgroundImage =
      "url('../img/carousel_background.jpg')"; // Replace with your actual background image URL
...
function renderShoeCards(shoes) {
  ...
  // Attach event listeners for "More Information" buttons
  document.querySelectorAll(".btn-success").forEach((button) => {
    button.addEventListener("click", function () {
      const index = this.getAttribute("data-index");
      const shoe = shoes[index]; // Get the correct shoe data
      populateModal(shoe); // Call the function to populate the modal
    });
  });
}
...
function populateModal(shoe) {
  ...
  // Remove the phrase "about this shoe" from the description if it exists
  let description = shoe.description.replace("about this shoe:", "").trim()
  
  // Update the description in the modal
  document.getElementById("modal-shoe-description").innerText = `Description: 
  ${description}`
}
